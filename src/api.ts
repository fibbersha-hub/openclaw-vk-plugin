...
export class VkApi {
  ...
  async uploadPhotoForMessage(filePath: string, peerId?: number): Promise<VkPhoto> {
    ...
    console.log("[vk-upload-debug] upload response:", JSON.stringify(uploaded));
    const photoValue = uploaded.photo || uploaded.photos_list;
    if (!photoValue) throw new Error(`VK upload returned no photo data: ${JSON.stringify(uploaded)}`);
  ...
  async uploadPhotoBufferForMessage(buffer: Buffer, filename: string, peerId?: number): Promise<VkPhoto> {
    const server = await this.photosGetMessagesUploadServer({ peer_id: peerId });
    const uploaded = await this.uploadFileFromBuffer(server.upload_url, buffer, filename, "photo");
    const saved = await this.photosSaveMessagesPhoto({
      photo: (uploaded.photo || uploaded.photos_list)!,
      server: uploaded.server,
      hash: uploaded.hash,
    });
    return saved[0]!;
