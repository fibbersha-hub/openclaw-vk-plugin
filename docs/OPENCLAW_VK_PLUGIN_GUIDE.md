...
openclaw channels vk allow YOUR_VK_USER_ID
```
    ...
    "vk": {
      "accounts": {
        "default": {
          ...
          "enabled": true,
          "dmPolicy": "pairing",
          "allowFrom": ["YOUR_VK_USER_ID", "YOUR_PARTNER_VK_ID"],
          "apiVersion": "5.199",
          "longPollWait": 25,
          "formatMarkdown": true,
          "autoKeyboard": true,
...
| `attachment` | `string` | Вложения: `photo-YOUR_GROUP_ID_123,doc-YOUR_GROUP_ID_456` |
...
| Поле | Описание |
|------|----------|
| `owner_id` | Отрицательный ID для сообщества (−YOUR_GROUP_ID) |
| `message` | Текст поста |
| `attachments` | Вложения через запятую: `photo-YOUR_GROUP_ID_123,video-YOUR_GROUP_ID_456` |
...
const result = await api.execute<unknown[]>(`
  return [
    API.groups.getById({"group_id": "YOUR_GROUP_ID"}),
    API.market.get({"owner_id": -YOUR_GROUP_ID, "count": 10}),
    API.wall.get({"owner_id": -YOUR_GROUP_ID, "count": 5})
  ];
`);

// Автоматический батчинг массива вызовов
const results = await api.executeBatch([
  { method: "groups.getById", params: { group_id: "YOUR_GROUP_ID" } },
  { method: "market.get", params: { owner_id: -YOUR_GROUP_ID, count: 10 } },
...
try {
  await api.messagesSend({ peer_id: YOUR_VK_USER_ID, random_id: ..., message: "Привет" });
} catch (e) {
...
| Тип | Формат | Пример |
|-----|--------|--------|
| Фото | `photo{owner_id}_{photo_id}` | `photo-YOUR_GROUP_ID_123456` |
| Документ | `doc{owner_id}_{doc_id}` | `doc-YOUR_GROUP_ID_789` |
| Видео | `video{owner_id}_{video_id}` | `video-YOUR_GROUP_ID_321` |
| Аудио | `audio{owner_id}_{audio_id}` | `audio123456_789` |
| Опрос | `poll{owner_id}_{poll_id}` | `poll-YOUR_GROUP_ID_456` |

Несколько вложений — через запятую: `photo-YOUR_GROUP_ID_123,doc-YOUR_GROUP_ID_456`
...
const api = new VkApi({ token: "vk1.a...", groupId: "YOUR_GROUP_ID" });
...
const [groupInfo, marketItems, wallPosts] = await api.executeBatch([
  { method: "groups.getById", params: { group_id: "YOUR_GROUP_ID", fields: "members_count,status" } },
  { method: "market.get", params: { owner_id: -YOUR_GROUP_ID, count: 10 } },
  { method: "wall.get", params: { owner_id: -YOUR_GROUP_ID, count: 5 } },
]);
```
